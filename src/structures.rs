...
use rust_xlsxwriter::{Format, FormatAlign, XlsxSerialize, serialize_option_datetime_to_excel};
use serde::{Deserialize, Deserializer, Serialize, de::Error};
use serde_json::Value;
use std::sync::LazyLock;

/// The base font size for all standard data cells in the workbook.
pub const FONT_SIZE: f64 = 14.0;

/// The specific font size applied exclusively to header cells.
pub const HEADER_FONT_SIZE: f64 = 12.0;

/// Central registry holding pre-computed cell and header styles.
///
/// This implements the Registry pattern, centralizing formatting rules to avoid
/// duplicating format building logic across multiple field attributes.
#[derive(Debug, Clone)]
pub struct FormatRegistry {
    /// Aesthetic configuration for worksheet headers.
    pub header: Format,
    /// Center-aligned standard data cells.
    pub centered: Format,
    /// Bold, center-aligned data cells (ideal for IDs and critical codes).
    pub bold_centered: Format,
    /// Currency format with thousand separators and decimals.
    pub currency: Format,
    /// Bold variant of the currency layout.
    pub bold_currency: Format,
    /// Date layout configured to standard Portuguese locale formats (dd/mm/yyyy).
    pub date: Format,
}

impl FormatRegistry {
    /// Computes and instantiates the full matrix of Excel formatting templates.
    pub fn new() -> Self {
        Self {
            header: Format::new()
                .set_text_wrap()
...
/// Global lazy-loaded thread-safe formatting registry.
///
/// Ensures styles are computed exactly once at runtime, maximizing execution
/// efficiency when processing large chunks of records.
pub static FORMAT: LazyLock<FormatRegistry> = LazyLock::new(FormatRegistry::new);

#[derive(Debug, Default, Serialize, Deserialize, XlsxSerialize)]
#[xlsx(table = Table::new())]
#[xlsx(header_format = FORMAT.header.clone())]
#[serde(rename_all = "PascalCase")]
pub struct PerDcomp {
    #[serde(rename = "PER/DCOMP")]
    #[xlsx(value_format = FORMAT.bold_centered.clone())]
    //#[xlsx(column_width = 30.0)]
    pub per_dcomp: Option<String>,

    #[serde(rename = "CNPJ/CPF Declarante/Sucessora")]
    #[xlsx(value_format = FORMAT.centered.clone())]
    ...
    #[xlsx(value_format = FORMAT.currency.clone())]
    //#[xlsx(column_width = 18.0)]
    ...
    #[xlsx(value_format = FORMAT.bold_currency.clone())]
    //#[xlsx(column_width = 18.0)]
    ...
    #[xlsx(value_format = FORMAT.date.clone())]
    //#[xlsx(column_width = 18.0)]
    ...
    #[xlsx(value_format = FORMAT.centered.clone())]
    //#[xlsx(column_width = 14.0)]
    pub trimestre_de_apuracao: Option<String>,

    #[serde(rename = "Ano")]
    #[xlsx(value_format = FORMAT.centered.clone())]
    //#[xlsx(column_width = 8.0)]
    pub ano: Option<u32>,
